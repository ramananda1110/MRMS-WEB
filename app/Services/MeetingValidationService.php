...
class MeetingValidationService
{
    ...
    public function validateMeetingUpdate($request, $existingMeeting)
    {
        $rules = [
            'meeting_title' => 'required|string|max:255',
            'host_id' => 'required|exists:employees,employee_id',
        ...
        $dateOrTimeOrRoomChanged = 
            ($request->has('room_id') && $request->input('room_id') != $existingMeeting->room_id) ||
            ($request->has('start_date') && $request->input('start_date') != $existingMeeting->start_date) ||
            ($request->has('start_time') && $request->input('start_time') != $existingMeeting->start_time) ||
            ($request->has('end_time') && $request->input('end_time') != $existingMeeting->end_time);

        if ($dateOrTimeOrRoomChanged) {
        ...
        $validator->after(function ($validator) use ($request, $existingMeeting, $dateOrTimeOrRoomChanged) {
            if ($dateOrTimeOrRoomChanged) {
                $room_id = $request->input('room_id', $existingMeeting->room_id);
                $start_date = $request->input('start_date', $existingMeeting->start_date);
                $start_time = $request->input('start_time', $existingMeeting->start_time);
                $end_time = $request->input('end_time', $existingMeeting->end_time);
                ...
                $overlappingMeeting = Meeting::where('room_id', $room_id)
                    ->where('start_date', $start_date)
                    ->where('booking_status', '!=', 'rejected')
                    ->where('id', '!=', $existingMeeting->id) // Exclude current meeting
                    ->where(function ($query) use ($updated_start_time, $end_time) {
                        $query->whereBetween('start_time', [$updated_start_time, $end_time])
                            ->orWhereBetween('end_time', [$updated_start_time, $end_time])
                            ->orWhere(function ($query) use ($updated_start_time, $end_time) {
